...
class CoCa(nn.Module):
    ...
    def _encode_image(self, images=None, normalize=True, return_tokens=False):
        """
        images: (B, C, H, W)
        return_tokens: nếu True, trả về token-level embeddings (B, N, D)
        """
        image_latent, tokens_embs = self.visual(images)  # tokens_embs: (B, N, D)
        
        if normalize:
            image_latent = F.normalize(image_latent, dim=-1)
            tokens_embs = F.normalize(tokens_embs, dim=-1)
        
        if return_tokens:
            return tokens_embs  # (B, N, D)
        else:
            return image_latent, tokens_embs
    ...
    # -------------------------
    def _encode_text(self, text, normalize=True, embed_cls=True):
        text_input = text[:, :-1] if embed_cls else text
        text_latent, token_embs = self.text(text_input)
        if normalize:
    ...
    def encode_image(self, images, normalize=True, return_tokens=False):
        """
        images: (B, C, H, W)
        return_tokens: nếu True, trả về N×D
        """
        feats = self._encode_image(images, normalize=normalize, return_tokens=return_tokens)
        return feats
    ...
    # -------------------------
    # Forward (giữ nguyên các tính năng khác)
    # -------------------------
    def forward(self, image, text, embed_cls=True, image_latent=None, image_embs=None, return_tokens=False):
        """
        return_tokens: nếu True, trả về token-level embeddings N×D
        """
        text_latent, token_embs = self._encode_text(text, embed_cls=embed_cls)
        ...
        return {
            "image_features": image_latent,  # (B, D)
            "text_features": text_latent,
            "logits": logits,
            "labels": labels,
            "logit_scale": self.logit_scale.exp()

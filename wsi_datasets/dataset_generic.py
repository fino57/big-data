		...
		if not self.use_h5:
			if self.data_dir_s and self.data_dir_l:
				if(self.mode == 'transformer'):
					...
					try:
						features_s = torch.load(os.path.join(data_dir_s, '{}.pt'.format(slide_id)), weights_only=True)
					except RuntimeError as e:
						error_msg = f"Error loading file: {slide_id}.pt. Error: {str(e)}"
						print(error_msg)  # Print to console
						raise e
					try:
						features_l = torch.load(os.path.join(data_dir_l, '{}.pt'.format(slide_id)), weights_only=True)
					except RuntimeError as e:
						error_msg = f"Error loading file: {slide_id}.pt. Error: {str(e)}"
						print(error_msg)  # Print to console
						raise e

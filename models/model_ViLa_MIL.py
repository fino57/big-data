...
class ViLa_MIL_Model(nn.Module):
    def __init__(self, config, num_classes=3):
        ...
        # Projector cho image features (768-dim từ backbone → L)
        self.projector = nn.Linear(768, config.input_size)

        # Load CLIP backbone cho text
        ...
        # 🔥 Lấy đúng dimension đầu ra text_features từ CLIP
        clip_text_dim = clip_model.text_projection.shape[1]  # RN50: 1024
        self.text_projector = nn.Linear(clip_text_dim, config.input_size)
        ...
        # Learnable image centers
        self.learnable_image_center = nn.Parameter(
            torch.Tensor(config.prototype_number, 1, config.input_size)
        )
        trunc_normal_(self.learnable_image_center, std=.02)

    def forward(self, x_s, x_l, label):
        # --- Project image features ---
        x_s, x_l = self.projector(x_s), self.projector(x_l)

        # --- Encode text ---
        prompts = self.prompt_learner()
        tokenized_prompts = self.prompt_learner.tokenized_prompts
        text_features = self.text_encoder(prompts, tokenized_prompts)   # [num_prompts, clip_text_dim]
        text_features = self.text_projector(text_features)              # [num_prompts, L]
